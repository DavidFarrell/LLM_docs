...
LX = [-5.3, -3.7, -2.1, -0.5]
BX, BW = -2.9, 6.4        # block centre x and width
Y_CHIP, Y_X, Y_MHA, Y_AN1, Y_FFN, Y_AN2, Y_OUT = -3.35, -2.55, -1.5, -0.42, 0.62, 1.66, 2.5
RP = 2.9                  # centre of the right-hand explanation panel


def hvec(vals, color, cell=0.19):
    return VecCells(vals, color, cell=cell, gap=0.03, direction=RIGHT)


def nn_diagram(n_in=6, n_hid=12, n_out=6, x0=1.9, x1=3.9, x2=5.9, h=3.2, y=0.0):
    def col(n, x, spread):
...
class S10_EncoderLayer(TScene):
    ...
    def construct(self):
        ...
        chips = VGroup(*[Chip(t, size=24, min_width=1.2).move_to([x, Y_CHIP, 0]) for t, x in zip(TOK, LX)])
        ...
        ffns = VGroup(*[Block("Feed\nForward", FFN_C, width=1.18, height=0.66, size=17).move_to([x, Y_FFN, 0])
                        for x in LX])
        ...
        def res_path(y0, y1):
            p = VMobject().set_points_as_corners([[BX + BW / 2 - 0.05, y0, 0], [0.75, y0, 0], [0.75, y1, 0],
                                                  [BX + BW / 2 + 0.02, y1, 0]])
        ...
        with self.vo("b1") as b:
            ...
            layer_box = DashedVMobject(RoundedRectangle(corner_radius=0.25, width=BW + 1.6, height=Y_AN2 - Y_MHA + 1.6),
                                       num_dashes=70).set_stroke(INK3, 1.5)
            layer_box.move_to([BX + 0.35, (Y_MHA + Y_AN2) / 2, 0])
            lb_lab = txt("one encoder layer", 22, INK3).next_to(layer_box, UP, buff=0.08).align_to(layer_box, LEFT)
        ...
        with self.vo("b2") as b:
            ...
            side1.move_to([RP + 1.3, Y_X + 0.3, 0])
            ...
            for i in range(4):
                for j in range(4):
                    ...
                    links.add(a)
            side2 = txt("words exchange information", 24, ATT_C).move_to([RP + 0.9, Y_MHA, 0])
            self.sfx("shimmer", -10)
        ...
        with self.vo("b4") as b:
            ...
            ln_t = txt("layer norm: re-centre and rescale", 26, NORM_C, weight="SEMIBOLD").move_to([RP + 0.8, 2.4, 0])
            ...
            fml.move_to([RP + 1.0, -2.7, 0])
            if fml.get_right()[0] > 6.8:
                fml.shift(LEFT * (fml.get_right()[0] - 6.8))
        ...
        with self.vo("b5") as b:
            ...
            nn = nn_diagram(y=0.1)
            e1, e2, na, nh, no = nn
            call = DashedLine(ffns[1].get_right(), [1.55, 0.1, 0], dash_length=0.08).set_stroke(FFN_C, 1.5)
            ttl = txt("feed-forward network", 26, FFN_C, weight="SEMIBOLD").move_to([RP + 1.0, 2.35, 0])
            ...
            self.play(ffns[1].rect.animate.set_fill(opacity=0.45), Create(call), FadeIn(ttl), run_time=0.5)
            ...
            up_t = txt("expand", 22, INK3).move_to([(1.9 + 3.9) / 2, 2.0, 0])
        ...
        with self.vo("b6") as b:
            ...
            self.play(FadeOut(nn_grp), ffns[1].rect.animate.set_fill(opacity=0.16), run_time=0.5)
            ...
            dl = txt("A division of labour", 32, INK, weight="SEMIBOLD").move_to([RP + 1.0, 2.3, 0])
            ...
            cg1 = VGroup(c1, c1s).arrange(DOWN, buff=0.15).move_to([RP + 1.0, 0.9, 0])
            ...
            cg2 = VGroup(c2, c2s).arrange(DOWN, buff=0.15).move_to([RP + 1.0, -0.6, 0])
        ...
        with self.vo("b7") as b:
            ...
            pt = txt("parameters in one encoder layer", 24, INK2).move_to([RP + 1.0, 2.35, 0])
            total_w = 5.2
            ...
            seg = VGroup(att_seg, ffn_seg).arrange(RIGHT, buff=0.04).move_to([RP + 1.0, 1.55, 0])
            ...
            nn2 = nn_diagram(n_in=5, n_hid=8, n_out=5, x0=2.6, x1=4.1, x2=5.6, h=1.5, y=-0.75)
            e1b, e2b, nab, nhb, nob = nn2
            memt = txt("later research: a key-value memory", 24, INK).move_to([RP + 1.1, 0.35, 0])
            cite = txt("Geva et al., 2021", 18, INK3).move_to([RP + 1.1, -3.05, 0])
            self.play(FadeIn(memt), FadeIn(nn2), FadeIn(cite), run_time=0.7)
            b.wait_until("fk", lead=0.2)
            fk_t = txt("layer 1 ≈ keys: pattern detectors", 22, K_C).move_to([RP + 1.1, -1.95, 0])
            ...
            fv_t = txt("layer 2 ≈ values: what to write out", 22, V_C).move_to([RP + 1.1, -2.35, 0])
            ...
            again = txt("the same idea, frozen into weights", 24, Q_C, weight="SEMIBOLD").move_to([RP + 1.1, -3.5, 0])
        ...
        with self.vo("b8") as b:
            ...
            full = txt("a complete encoder layer", 28, INK, weight="SEMIBOLD").move_to([RP + 1.1, 0.3, 0])
            ...
            later = txt("later layers see vectors already", 24, INK).move_to([RP + 1.4, 1.0, 0])
        ...
        with self.vo("b9") as b:
            ...
            mats.arrange_in_grid(2, 3, buff=(0.5, 0.35)).move_to([RP + 1.3, 1.4, 0])
